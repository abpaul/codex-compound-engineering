...
This skill is the Codex equivalent of `/generate_command` from Every's compound-engineering plugin.
Invoke it with `$generate-command`.
...
1. **Create the file** at `.agents/commands/[name].md` (subdirectories like `workflows/` supported)
