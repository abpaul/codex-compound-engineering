...
| **Team** | A named group of agents working together. One leader, multiple teammates. | `~/.agents/teams/{name}/config.json` |
...
| **Task** | A work item with subject, description, status, owner, and dependencies. | `~/.agents/tasks/{team}/N.json` |
| **Inbox** | JSON file where an agent receives messages from teammates. | `~/.agents/teams/{name}/inboxes/{agent}.json` |
...
// Rails code review
Task({
  subagent_type: "compound-engineering:review:dhh-rails-style",
...
- `dhh-rails-style` - DHH/37signals Rails style
...
**Creates:**
- `~/.agents/teams/feature-auth/config.json`
- `~/.agents/tasks/feature-auth/` directory
...
**Removes:**
- `~/.agents/teams/{team-name}/` directory
- `~/.agents/tasks/{team-name}/` directory
...
// cat ~/.agents/teams/code-review/inboxes/team-lead.json
...
A **backend** determines how teammate Claude instances actually run. Codex supports three backends, and **auto-detects** the best one based on your environment.
...
**How it works:**
- Uses iTerm2's automation API via `it2` CLI
...
- automation API enabled in iTerm2 preferences
...
**Cons:**
- macOS + iTerm2 only
- Requires setup (it2 CLI + automation API)
...
# 2. Enable automation API in iTerm2
# iTerm2 → Settings → General → Magic → Enable automation API
...
| "Automation API not enabled" | it2 can't communicate with iTerm2 | Enable in iTerm2 Settings → General → Magic |
...
cat ~/.agents/teams/{team}/config.json | jq '.members[].backendType'
...
// Read ~/.agents/teams/{team}/config.json
...
```bash
# Check team config
cat ~/.agents/teams/{team}/config.json | jq '.members[] | {name, agentType, backendType}'

# Check teammate inboxes
cat ~/.agents/teams/{team}/inboxes/{agent}.json | jq '.'

# List all teams
ls ~/.agents/teams/

# Check task states
cat ~/.agents/tasks/{team}/*.json | jq '{id, subject, status, owner, blockedBy}'

# Watch for new messages
tail -f ~/.agents/teams/{team}/inboxes/team-lead.json
```
...
// cat ~/.agents/teams/pr-review-123/inboxes/team-lead.json
...
```bash
cat ~/.agents/teams/{team}/inboxes/team-lead.json | jq '.'
```
