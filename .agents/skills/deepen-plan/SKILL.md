...
This skill is the Codex equivalent of `/deepen-plan` from Every's compound-engineering plugin.
Invoke it with `$deepen-plan`.
...
- [ ] Technologies/frameworks mentioned (Rails, React, TypeScript, etc.)
...
# 2. User's global skills (~/.agents/)
ls ~/.agents/skills/

# 3. compound-engineering plugin skills
ls ~/.agents/plugins/cache/*/compound-engineering/*/skills/

# 4. ALL other installed plugins - check every plugin for skills
find ~/.agents/plugins/cache -type d -name "skills" 2>/dev/null

# 5. Also check installed_plugins.json for all plugin locations
cat ~/.agents/plugins/installed_plugins.json
```
...
```
Launch a general subagent with Task: "You have the [skill-name] skill available at [skill-path].
...
**Example spawns:**
```
Launch a general subagent with Task: "Use $dhh-rails-style. Read SKILL.md and apply it to: [Rails sections of plan]"

Launch a general subagent with Task: "Use $frontend-design. Read SKILL.md and apply it to: [UI sections of plan]"

Launch a general subagent with Task: "Use $agent-native-architecture. Read SKILL.md and apply it to: [agent/tool sections of plan]"

Launch a general subagent with Task: "Use $security-sentinel. Read SKILL.md and apply it to: [security sections of plan]"
```
...
find .agents/docs -name "*.md" -type f 2>/dev/null
find ~/.agents/docs -name "*.md" -type f 2>/dev/null
```
...
- Plan is non-Rails → skip `rails-specific/` learnings
...
```
Launch a general subagent with Task: "
...
```
Launch an explore subagent with Task: "Research best practices, patterns, and real-world examples for: [section topic].
Find:
...
```
context7 resolve-library-id: Find library ID for [framework]
context7 query-docs: Query documentation for specific patterns
```
...
find .agents/agents -name "*.md" 2>/dev/null

# 2. User's global agents (~/.agents/)
find ~/.agents/agents -name "*.md" 2>/dev/null

# 3. compound-engineering plugin agents (all subdirectories)
find ~/.agents/plugins/cache/*/compound-engineering/*/agents -name "*.md" 2>/dev/null

# 4. ALL other installed plugins - check every plugin for agents
find ~/.agents/plugins/cache -path "*/agents/*.md" 2>/dev/null
...
2. **Run `$technical-review`** - Get feedback from reviewers on enhanced plan
...
- **`$technical-review`** → Call the $technical-review command with the plan file path
