...
This skill is the Codex equivalent of `/test-xcode` from Every's compound-engineering plugin.
Invoke it with `$test-xcode`.
...
Then restart Codex and run `$test-xcode` again.
```
...
```
Launch a general subagent with Task: "Run $test-xcode for scheme [name]. Build, install on simulator, test key screens, check for crashes."
```
